...
def process_customer_data_enhanced(csv_path):
    ...
    df["Dt_Customer"] = pd.to_datetime(
        df["Dt_Customer"], errors="coerce", dayfirst=True
    )
    df["customer_joining_year"] = df["Dt_Customer"].dt.year
    ...
    df["Age_Group"] = pd.cut(
        df["Age"], bins=[0, 35, 55, 100], labels=["Young", "Middle", "Senior"]
    )
    ...
    df["Income_Quintile"] = pd.qcut(df["Income"], 5, duplicates="drop")
    df["Is_High_Income"] = (df["Income"] > df["Income"].quantile(0.75)).astype(int)

    # Spending patterns
    df["Total_Spending"] = df["MntProds"]  # Already calculated
    df["Spending_Per_Income"] = df["Total_Spending"] / (
        df["Income"] + 1
    )  # +1 to avoid division by zero
    df["Spending_Per_Person"] = df["Total_Spending"] / df["Family_Size"]
    df["Avg_Monthly_Spending"] = df["Total_Spending"] / (
        df["years_since_joining_company"] * 12 + 1
    )
    ...
    df["Essential_Spending"] = (
        df["MntFruits"] + df["MntMeatProducts"] + df["MntFishProducts"]
    )
    ...
    df["Catalog_Purchase_Ratio"] = df["NumCatalogPurchases"] / (
        df["NumAllPurchases"] + 1
    )
    ...
    df["Is_Deal_Hunter"] = (
        df["Deal_Sensitivity"] > df["Deal_Sensitivity"].median()
    ).astype(int)

    # Purchase frequency
    df["Avg_Purchase_Frequency"] = df["NumAllPurchases"] / (
        df["years_since_joining_company"] + 1
    )
    df["Days_Between_Purchases"] = (365 * df["years_since_joining_company"]) / (
        df["NumAllPurchases"] + 1
    )
    ...
    df["Web_Visits_Per_Purchase"] = df["NumWebVisitsMonth"] / (
    ...
    df["Loyalty_Score"] = (
        df["years_since_joining_company"]
        * (1 - df["Complain"])
    ...
    # Spending tiers
    df["Spending_Tier"] = pd.qcut(df["Total_Spending"], 3, duplicates="drop")

    # Income tiers
    df["Income_Tier"] = pd.qcut(df["Income"], 3, duplicates="drop")
    ...
    def get_customer_type(row):
        if row["Web_Purchase_Ratio"] > 0.5 and row["Total_Spending"] > row[
            "Total_Spending"
        ].quantile(0.75):
            return "High_Value_Web"
        elif row["Store_Purchase_Ratio"] > 0.5 and row["Total_Spending"] > row[
            "Total_Spending"
        ].quantile(0.75):
    ...
    bool_columns = [
        ...
        or col.startswith("Generation_")
        or col.startswith("Income_Quintile_")
        or col.startswith("Spending_Tier_")
        or col.startswith("Income_Tier_")
        or col.startswith("Life_Stage_")
        or col.startswith("Customer_Type_")
        or col.startswith("Recency_Group_")
        or col.startswith("Preferred_Channel_")
    ]
    ...
    columns_to_drop = ["ID", "Education", "Dt_Customer", "customer_joining_year"]
...
def prepare_clustering_data(df):
    ...
    all_data = pd.concat(
        [
            pd.DataFrame(scaled_num_data, columns=num_data.columns),
